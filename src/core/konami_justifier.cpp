...
void KonamiJustifier::UpdatePosition()
{
  ...
  {
    Log_DevPrintf("Lightgun window coordinates %d,%d -> tick %u line %u", mouse_x, mouse_y, m_position_tick,
                  m_position_line);
...
void KonamiJustifier::UpdateIRQEvent()
{
  ...
  if (!m_position_valid || IsTriggerPressed())
    return;

  u32 target_line = m_position_line + m_irq_repeats;
  m_irq_repeats++;

  if (target_line >= g_gpu->GetCRTCActiveEndLine() || m_irq_repeats == NUM_IRQ_REPEATS)
  {
    target_line = m_position_line;
    m_irq_repeats = 0;
  }
